...
def __confidence_loss(y_true, y_pred, mask, alpha, gamma):
    ...
    ale = AbsoluteLogarithmicError(alpha=0.5, gamma=0.0)
    loss = tf.reduce_mean(ale(obj_true, obj_pred) * mask[:, :, :, 0])
...
def __bbox_loss(y_true, y_pred, mask):
    ...
    return loss / obj_count / tf.cast(tf.shape(y_true)[0], dtype=y_pred.dtype)
...
def __classification_loss(y_true, y_pred, mask, alpha, gamma, label_smoothing):
    ...
    class_true = y_true[:, :, :, 5:]
    class_pred = y_pred[:, :, :, 5:]
    num_classes = tf.cast(tf.shape(class_true)[0], y_pred.dtype)
    # ale = AbsoluteLogarithmicError(alpha=alpha, gamma=gamma, label_smoothing=label_smoothing)
    ale = AbsoluteLogarithmicError(alpha=0.5, gamma=0.0, label_smoothing=label_smoothing)
    loss = tf.reduce_sum(tf.reduce_sum(ale(class_true, class_pred), axis=-1) * obj_true * mask[:, :, :, 0])
    return loss / obj_count / num_classes / tf.cast(tf.shape(y_true)[0], dtype=y_pred.dtype)
