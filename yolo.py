...
class YoloLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        wh_true = tf.sqrt(y_true[:, :, :, 3:5] + 1e-4)
        wh_pred = tf.sqrt(y_pred[:, :, :, 3:5] + 1e-4)
        wh_loss = tf.reduce_mean(tf.reduce_mean(tf.reduce_mean(tf.abs(wh_true - wh_pred), axis=-1) * y_true[:, :, :, 0]))
        wh_loss = -tf.math.log(1.0 + 1e-7 - wh_loss)
        ...
        return confidence_loss + xy_loss + wh_loss + classification_loss
